...
def get_tiny_images(image_paths):
    ...
    image_arr = np.zeros((len(image_paths), 256))
    for i in range (len(image_paths)):
        image_arr[i] = (resize(rgb2grey(io.imread(image_paths[i])), (16, 16), anti_aliasing=True)).flatten()
    return image_arr
...
def build_vocabulary(image_paths, vocab_size):
    ...
    for i in image_paths:
        image = rgb2grey(io.imread(i))
        hog_descriptor = hog(image, pixels_per_cell=(8, 8), cells_per_block=(4,4), feature_vector=True)
        reshaped_hog = hog_descriptor.reshape(-1, 144)
        for hog_feat in reshaped_hog:
            vocab.append(hog_feat)
    kmeans = MiniBatchKMeans(n_clusters=vocab_size, max_iter=100)
    kmeans = kmeans.fit(vocab)
...
def get_bags_of_words(image_paths):
    ...
    #TODO: Implement this function!
    histo_arr = np.empty((len(image_paths), len(vocab)))
    image_count = 0
    for i in image_paths:
        image = rgb2grey(io.imread(i))
        hog_descriptor = hog(image, pixels_per_cell=(8, 8), cells_per_block=(4,4), feature_vector=True)
        reshaped_hog = hog_descriptor.reshape(-1, 144)
        distance = cdist(vocab, reshaped_hog)
        sorted_dist = np.argsort(distance, axis=0)
        sorted_m = sorted_dist[0, :]
        histo = np.histogram(sorted_m, bins=len(vocab))[0]
        histo = histo/np.linalg.norm(histo)
        histo_arr[image_count] = histo
        image_count += 1
    return histo_arr
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    '''
    #look at auto argument 
    # TODO: Implement this function!
    # SVC(kernel = 'linear').fit(train_image_feats, train_labels).predict(test_image_feats)
    return SVC(kernel = 'rbf').fit(train_image_feats, train_labels).predict(test_image_feats)
    # SVC(kernel = 'poly', degree=3).fit(train_image_feats, train_labels).predict(test_image_feats)
    # LinearSVC(penalty = 'l1', dual=False).fit(train_image_feats, train_labels).predict(test_image_feats)
    # return LinearSVC().fit(train_image_feats, train_labels).predict(test_image_feats)
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    # sorted_dist_val = np.sort(distances, axis=1)

    sorted_k = sorted_dist[:, 0:k]
    # sorted_k_val = sorted_dist_val[:, 0:k]
    sorted_labels = []
    for i in sorted_k:
        label_arr = []
        for j in i:
            label_arr.append(train_labels[j])
        sorted_labels.append(mode(label_arr, axis=None)[0][0])
    return sorted_labels


#vocab = 200: 60.733%,  12 min 53s total runtime SVM/bag
#             50.933%             nn/bag
#tiny nn     20.256%    about 20 s     
#tiny SVM     30.800%   

#vocab = 10: 47.333%, 5 min
#vocab = 20: 52.133%, 4.5 min
#vocab = 50: 55.067%
#vocab = 100: 54.733%
#vocab = 500: 52.067%