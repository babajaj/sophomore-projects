...
import edu.brown.cs.ilayzer.tIMDb.actors.ActorEdge;
import edu.brown.cs.ilayzer.tIMDb.actors.ActorGraph;
import edu.brown.cs.ilayzer.tIMDb.actors.ActorNode;
...
public class Connect implements Executable {
  ...
  private ActorDatabase database = null;
   ...
   */
  public void setDatabase(ActorDatabase database) {
  ...
  public String execute(String[] args) {
    ...
    graph.setStartNode(new ActorNode(startIds[0], names[0]));
    Dijkstra<ActorNode, ActorEdge> dijkstra =
            new Dijkstra<>(graph, new DijkstraComparator<>());
    ...
    if (targetNode == null) {
      ...
      guiOutput = startIds[0].replace('/', '$') + "-/-" + names[0] + "-/-"
  ...
  /**
   * Gets the string output for the GUI.
   * @return the gui output
   */
  public String getGuiOutput() {
    String toReturn = this.guiOutput;
    this.guiOutput = null;
    return toReturn;
  }
...
//    }
  ...
  public void getDijkstraOutput(ActorNode curr) {
    ...
    output += prev.getActorName() + " -> " + curr.getActorName() + " : " + movie + "\n";
    //this is the output for the gui
    guiOutput +=
            prev.getId().replace('/', '$') + "--" + prev.getActorName()
                    + "--" + curr.getId().replace('/', '$') + "--"
                    + curr.getActorName() + "--"
