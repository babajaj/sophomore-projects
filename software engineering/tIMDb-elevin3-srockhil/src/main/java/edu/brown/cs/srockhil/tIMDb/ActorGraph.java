 ...
 */
public class ActorGraph implements Graph<ActorNode> {

  private Database database;
  private HashMap<String, ActorNode> nodes;
  private HashMap<Edge, ActorNode[]> neighbors;
   ...
   */
  public ActorGraph(Database database) {
  ...
  public List<ActorNode> expand(ActorNode actorNode) throws Exception {
    ...
    List<String> movies =
            database.query("aidToMids", actorNode.getElement());
    String currFullName =
            database.query("aidToName", actorNode.getElement()).get(0);
   ...
   */
  @Override
  public ActorNode getNeighbor(Edge edge, ActorNode node) {
   ...
   * @param edge     the edge connecting the two nodes
   */
  public void setPrev(ActorNode currNode, ActorNode prevNode, Edge edge) {
   ...
   */
  public void setStartNode(String id, String name) {
    startNode = new ActorNode(id, name);
    startNode.setValue(0);
    nodes.put(id, startNode);
  }
